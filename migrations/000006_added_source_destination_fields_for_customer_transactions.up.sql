...
ALTER TABLE `tblCustomerLoan` ADD `source` VARCHAR(50) NULL AFTER `loan_application_id`;
ALTER TABLE `tblCustomerLoanApplication` ADD `source` VARCHAR(50) NULL AFTER `status`;
ALTER TABLE `tblCustomerLoanHistory` ADD `source` VARCHAR(50) NULL AFTER `status`;

ALTER TABLE `tblCustomerPayment` ADD `destination` VARCHAR(50) NULL AFTER `reference_code`;
ALTER TABLE `tblCustomerPaymentHistory` ADD `destination` VARCHAR(50) NULL AFTER `reference_code`;
...
CREATE TRIGGER `trg_after_tblCustomerLoanApplication_insert` AFTER INSERT ON `tblCustomerLoanApplication` FOR EACH ROW BEGIN
    DECLARE _balance DOUBLE(10,2);
	  IF (NEW.status="APPROVED") THEN      
		INSERT INTO tblCustomerLoan (fk_customer_id,loan_application_id,source,epoint_transaction_id,loan_amount,interest_amount,fee_amount,total_amount, reference_code,due_date,loan_date,approved_by,approved_date) VALUES (NEW.fk_customer_id,NEW.id,NEW.source,NEW.epoint_transaction_id, NEW.loan_amount, NEW.interest_amount,NEW.fee_amount,NEW.total_amount,NEW.reference_code,NEW.due_date,NEW.loan_date,NEW.processed_by,NEW.processed_date);
		...
		insert into tblCustomerLoanTransaction (fk_customer_id,`type`,debit,balance) VALUES (NEW.fk_customer_id,"LOAN",NEW.total_amount,_balance);
          ...
          INSERT INTO tblCustomerLoanHistory (fk_customer_id,loan_amount,interest_amount,fee_amount,total_amount,reference_code,due_date,loan_date,`status`,source,epoint_transaction_id,processed_by,processed_date) 
          VALUES (NEW.fk_customer_id,NEW.loan_amount,NEW.interest_amount,NEW.fee_amount,NEW.total_amount,NEW.reference_code,NEW.due_date,NEW.loan_date,NEW.status,NEW.source,NEW.epoint_transaction_id,NEW.processed_by,NEW.processed_date);
    END;
...
ALTER ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `view_transaction_history`  AS  select `tblCustomerLoanHistory`.`fk_customer_id` AS `fk_customer_id`,`tblCustomerLoanHistory`.`source` AS `mode`,`tblCustomerLoanHistory`.`loan_amount` AS `amount`,'LOAN' AS `t_type`,`tblCustomerLoanHistory`.`loan_date` AS `t_date` from `tblCustomerLoanHistory` union select `tblCustomerPaymentHistory`.`fk_customer_id` AS `fk_customer_id`,`tblCustomerPaymentHistory`.`destination` AS `mode`,`tblCustomerPaymentHistory`.`payment_amount` AS `amount`,'PAYMENT' AS `t_type`,`tblCustomerPaymentHistory`.`date_paid` AS `t_date` from `tblCustomerPaymentHistory`;

...
type (
	...
	KeyAuthConfig struct {
		...
		// CustomerUniqueIDFieldName passes customer unique ID field name for POST requests
		CustomerUniqueIDFieldName string
	}

	// KeyAuthValidator defines a function to validate KeyAuth credentials.
	KeyAuthValidator func(string, string, echo.Context) (bool, error)
...
func KeyAuthWithConfig(config KeyAuthConfig) echo.MiddlewareFunc {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			valid, err := config.Validator(key, config.CustomerUniqueIDFieldName, c)
			if err != nil {
				return err
			} else if valid {
				return next(c)
